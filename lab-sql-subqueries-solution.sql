...
join (select actor_id, count(film_id) from sakila.film_actor
group by actor_id
order by count(film_id) desc
limit 1) a 
using (actor_id);
...
select f.title from sakila.film f
join (select fa.film_id from sakila.film_actor fa
join (select actor_id, count(film_id) from sakila.film_actor
group by actor_id
order by count(film_id) desc
limit 1) a 
...
join (select inventory_id from sakila.rental
where customer_id = (select customer_id from sakila.payment
group by customer_id
order by sum(amount) desc
limit 1)) r
...
select f.title from sakila.film f
join (select i.film_id from sakila.inventory i
join (select inventory_id from sakila.rental
where customer_id = (select customer_id from sakila.payment
group by customer_id
order by sum(amount) desc
limit 1)) r
using (inventory_id)) fi
using (film_id);
...
select customer_id, sum(amount) as total_amount from sakila.payment
group by customer_id
having total_amount > (select avg(ta.total_amount) from (
select customer_id, sum(amount) as total_amount from sakila.payment
group by customer_id) as ta)
